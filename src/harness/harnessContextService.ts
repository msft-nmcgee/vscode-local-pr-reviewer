...
export function renderHarnessContext(manifest: HarnessManifest): string {
    const lines: string[] = [
        ...
        'Do not modify `.git/ai-review/` files directly. The extension owns review state.',
        '',
...
export function renderHarnessAgentInstructions(): string {
    return [
        ...
        '',
        '1. `.git/ai-review/harness-manifest.json`',
        '2. `.git/ai-review/harness-context.md`',
        '3. `.git/ai-review/active-feedback.md` when it exists',
        '4. `.git/ai-review/agent-reviews/*.json` when referenced by the manifest',
        '',
        'Do not modify files under `.git/ai-review/`. The VS Code extension owns review state.',
        '',
        '## Reconciliation task',
        '',
